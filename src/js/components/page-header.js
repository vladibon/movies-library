...
function goNavLink(e) {
    if (e.target.classList.contains('site-nav__link--active') === false) {
        ...
        if (e.target.dataset.nav === "home") {
            getSectionHeader.innerHTML = `<div class="page-header__search" ><input class="page-header__search--input" type="input" placeholder="Поиск фильмов" /><svg class="search-img" width="12" height="12"><use href="${svgURL}#icon-search"></use></svg></div >`;
        } else {
            getSectionHeader.innerHTML = '<div class="page-header__buttons"><!-- Site Nav Button --><button class="btn btn--primary btn__margin js-btn" type="button" data-action="watched">WATCHED</button><button class="btn btn--primary js-btn" type="button" data-action="queue">QUEUE</button></div>';
        }
    }
}
